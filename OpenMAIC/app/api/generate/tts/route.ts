...
// CPU VoxCPM inference (clone mode with reference audio) routinely takes
// 25-50 minutes per request. A 10-action scene queues ~10 of these back-to-back
// on the single-threaded CPU model, so we need a ceiling that survives the
// worst case. Bumped from 1500s to 3600s (60 min). The pre-gen TTS path also
// forces `denoise: false` below (see use-scene-generator.ts) because the
// reference audio is user-recorded and already clean — denoise adds 600+ extra
// ZipEnhancer steps (~25 min) per request with no quality benefit here.
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // For VoxCPM, the backend selection is a server-managed routing concern.
    // The client store may default to `vllm-omni`, but the actual deployment
    // may be `python-api`. Always prefer the server-side env value when set
    // so the client cannot route a managed VoxCPM at the wrong endpoint (#).
    let resolvedProviderOptions = ttsProviderOptions;
    if (ttsProviderId === VOXCPM_TTS_PROVIDER_ID) {
      const envBackend = process.env.TTS_VOXCPM_BACKEND?.trim() || '';
      if (envBackend) {
        const clientBackend =
          typeof ttsProviderOptions?.backend === 'string'
            ? (ttsProviderOptions.backend as string)
            : '';
        if (clientBackend !== envBackend) {
          resolvedProviderOptions = {
            ...(ttsProviderOptions || {}),
            backend: envBackend,
          };
          log.info(
            `VoxCPM backend override: client=${clientBackend || 'unset'} -> env=${envBackend}`,
          );
        }
      }
    }
