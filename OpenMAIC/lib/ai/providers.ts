...
// LLM calls (Qiniu DeepSeek/etc.) routinely take 5+ min for long scene content
// with thinking enabled. Node's built-in undici `globalThis.fetch` is capped at
// a 5min `headersTimeout`, which surfaces as "AI_APICallError: Cannot connect
// to API: Headers Timeout Error" inside @ai-sdk/openai and breaks long scenes
// with `POST /api/generate/scene-content 500 in 5.0min`. Re-route LLM calls
// through undici with a custom dispatcher that lifts the 5min ceiling to 24
// minutes (matches the route's maxDuration of 1500s).
//
// undici is dynamically imported (and guarded by `typeof window === 'undefined'`)
// because the same module is reachable from the client bundle via settings.ts
// → server-providers-init.tsx, and undici's transitive `node:assert` imports
// break webpack's client-side `node:` scheme handling even with
// `serverExternalPackages`.
type UndiciModule = typeof import('undici');
let _undiciPromise: Promise<UndiciModule> | undefined;
function loadUndici(): Promise<UndiciModule> {
  if (typeof window !== 'undefined') {
    return Promise.reject(new Error('undici is server-only'));
  }
  if (!_undiciPromise) {
    // webpackIgnore keeps the client bundle from trying to resolve undici
    // (its transitive `node:assert` import breaks webpack's `node:` scheme
    // handling). On the server, this resolves normally to the real module.
    _undiciPromise = import(/* webpackIgnore: true */ 'undici');
  }
  return _undiciPromise;
}
let _llmDispatcher: import('undici').Agent | undefined;
async function getLlmDispatcher(): Promise<import('undici').Agent> {
  if (_llmDispatcher) return _llmDispatcher;
  const { Agent } = await loadUndici();
  _llmDispatcher = new Agent({
    headersTimeout: 24 * 60 * 1000,
    bodyTimeout: 0,
