...
async function generateVoxCPMTTS(
  ...
  // DEBUG
  console.log('[voxcpm-debug] baseUrl=' + baseUrl + ' backend=' + backend + ' opts=' + JSON.stringify(options).slice(0, 300));
  const voicePrompt =
    options.voicePrompt ||
...
// Next.js 16 dev wraps `globalThis.fetch` with a hard 5min ceiling that
// surfaces as `TypeError: fetch failed` long before our own AbortSignal fires
// (setGlobalDispatcher installed after the dev server has already wired its
// own proxy does not lift this cap). For routes that legitimately need >
// 5 min (CPU VoxCPM CPU inference routinely runs 25-50 min/action in clone
// mode), we go around the wrapped fetch entirely: build a multipart
// body ourselves, then call undici's fetch directly with a custom dispatcher
// that lifts headers/body/connect timeouts to an hour.
//
// The dynamic import with `webpackIgnore` keeps the client bundle from
// trying to resolve undici's transitive `node:assert` imports — only the
// server runtime path executes this code.
function buildMultipartBody(
  fields: Array<{ name: string; value: string | Blob; filename?: string }>,
): Promise<{ body: Uint8Array; contentType: string }> {
  const boundary = '----OpenMAICTTS' + Math.random().toString(36).slice(2);
  return (async () => {
    const chunks: Buffer[] = [];
      ...
      chunks.push(
        Buffer.from(
          `--${boundary}\r\nContent-Disposition: form-data; name="${f.name}"${
            ...
            f.value instanceof Blob && f.value.type
              ? `\r\nContent-Type: ${f.value.type}`
              : ''
          }\r\n\r\n`,
        ),
      );
      if (typeof f.value === 'string') {
        chunks.push(Buffer.from(f.value, 'utf8'));
      } else {
        // Blob → ArrayBuffer. The only portable synchronous-looking path
        // across all Node 18.x and 20.x patch releases is the async
        // arrayBuffer() method, so we await it. The whole builder runs
        // inside an async IIFE so the caller can `await` the result.
        const buf = Buffer.from(await (f.value as unknown as Blob).arrayBuffer());
        chunks.push(buf);
      }
...
let _ttsUndiciPromise: Promise<typeof import('undici')> | undefined;
async function getUndici(): Promise<typeof import('undici')> {
  if (typeof window !== 'undefined') {
    throw new Error('undici is server-only');
  }
  if (!_ttsUndiciPromise) {
    _ttsUndiciPromise = import(/* webpackIgnore: true */ 'undici');
  }
  return _ttsUndiciPromise;
}
let _ttsDispatcher: import('undici').Agent | undefined;
async function getTtsDispatcher(): Promise<{
  dispatcher: import('undici').Agent;
  fetch: typeof import('undici').fetch;
}> {
  const { Agent, fetch: undiciFetch } = await getUndici();
  if (!_ttsDispatcher) {
    _ttsDispatcher = new Agent({
      headersTimeout: 60 * 60 * 1000,
      bodyTimeout: 0,
      connectTimeout: 30 * 1000,
    });
  }
  return { dispatcher: _ttsDispatcher, fetch: undiciFetch };
}
export async function postVoxCPMPythonAPI(
  baseUrl: string,
  params: {
    targetText: string;
    promptText?: string;
  ...
  // Bypass the dev-wrapped globalThis.fetch (5min ceiling) by building the
  // multipart body ourselves and posting through undici directly with a
  // dispatcher that lifts timeouts to an hour. AbortSignal remains as a
  // backstop in case some other proxy in the chain adds its own cap.
  const fields: Array<{ name: string; value: string | Blob; filename?: string }> = [
    { name: 'text', value: params.targetText },
    { name: 'cfg_value', value: String(params.cfgValue) },
    { name: 'inference_timesteps', value: String(params.inferenceTimesteps) },
    { name: 'normalize', value: String(params.normalize) },
    { name: 'denoise', value: String(params.denoise) },
  ];
  if (params.referenceAudioBase64) {
    const audioBlob = base64ToBlob(params.referenceAudioBase64, params.referenceAudioMimeType);
    const audioName = params.referenceAudioName || 'reference.wav';
    fields.push({ name: 'reference_audio', value: audioBlob, filename: audioName });
    if (params.promptText?.trim()) {
      fields.push({ name: 'prompt_audio', value: audioBlob, filename: audioName });
      fields.push({ name: 'prompt_text', value: params.promptText.trim() });
    }
  }
  const { body, contentType } = await buildMultipartBody(fields);
  const headers: Record<string, string> = {
    'Content-Type': contentType,
    'Content-Length': String(body.byteLength),
    ...getBackendAuthHeaders(apiKey),
  };
  // CPU VoxCPM clone mode routinely runs 30-50 min/action on the dev machine.
  // Cap the dispatcher at 60 min so a single fetch can survive the full
  // worst-case. AbortSignal stays as a defensive backstop.
  const FETCH_TIMEOUT_MS = 60 * 60 * 1000;
  const { dispatcher, fetch: undiciFetch } = await getTtsDispatcher();
  return undiciFetch(`${baseUrl}/tts/upload`, {
    method: 'POST',
    headers,
    body,
