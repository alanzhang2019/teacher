...
export function useDiscussionTTS({ enabled, agents, onAudioStateChange }: DiscussionTTSOptions) {
  ...
  const resolveVoiceForAgent = useCallback(
    (agentId: string | null): ResolvedVoice | null => {
      ...
      // Teacher's voice prefers the per-agent pick the user made in AgentBar
      // (a cloned voice, e.g. `voxcpm:profile:...`) so discussion teacher
      // matches the cloned voice used by pre-gen narration. Otherwise fall
      // back to the global lecture selection, honored VERBATIM (incl. its
      // model) whenever that provider is enabled — identical to what the
      // pre-generated lecture sends. Only if the global provider is itself
      // disabled does the teacher fall back to an enabled provider.
      if (agent.role === 'teacher') {
        const teacherOverride = agentVoiceOverrides?.[agent.id];
        if (
          teacherOverride &&
          isTTSProviderEnabled(teacherOverride.providerId, ttsProvidersConfig[teacherOverride.providerId])
        ) {
          return {
            providerId: teacherOverride.providerId,
            voiceId: teacherOverride.voiceId,
            modelId: teacherOverride.modelId,
          };
        }
