...
export async function generateAndStoreTTS(
  ...
  // Honor the per-agent voice pick the user made in AgentBar (e.g. a cloned
  // voice for the teacher). Otherwise pre-gen narration silently falls back to
  // `voxcpm:auto` and derives a different voice prompt per agent persona,
  // making every teacher sound different even when the user picked one voice.
  const teacherOverride = teacher
    ? settings.agentVoiceOverrides?.[teacher.id]
    : undefined;
  ...
  // endpoint is polled in a separate background loop (also fire-and-forget
  // so it can never block scene generation); when the task lands in
  // `completed`, we fetch the audio and write it into IndexedDB.
  const taskId = await submitBackgroundTTS(
    text,
    audioId,
    narrationVoiceId,
    providerOptions,
  ...
  if (!taskId) {
    // Fallback: queue was disabled or unreachable — log and move on; classroom
    // will fall back to `/audio-cache/<audioId>.wav` if it ever shows up.
    log.warn(`[TTS-bg] enqueue returned no taskId for ${audioId}`);
    return;
  }
  void pollAndStoreBackgroundTTS(taskId, audioId, signal);
}

/**
 * POST a TTS job to /api/generate/tts-background and get back a taskId.
 * Returns null if the server is unreachable or the response is malformed.
 */
async function submitBackgroundTTS(
  text: string,
  audioId: string,
  ...
  const ttsProviderConfig = settings.ttsProvidersConfig?.[settings.ttsProviderId];
  try {
    const response = await fetch('/api/generate/tts-background', {
      method: 'POST',
    ...
    const data = (await readJsonResponse(response)) as { success?: boolean; taskId?: string };
    if (!response.ok || !data?.taskId) return null;
    return data.taskId;
  } catch (err) {
    log.warn(`[TTS-bg] enqueue failed for ${audioId}:`, err);
    return null;
  }
}

/**
 * Poll the queue's status endpoint until the task completes (or fails), then
 * fetch the synthesized audio from `/audio-cache/<taskId>.wav` and write it
 * into IndexedDB so the classroom can play it through the normal audio path.
 *
 * Polling is fire-and-forget — never throws, never blocks the caller.
 */
async function pollAndStoreBackgroundTTS(
  taskId: string,
  audioId: string,
  signal?: AbortSignal,
): Promise<void> {
  const POLL_INTERVAL_MS = 5_000;
  const POLL_TIMEOUT_MS = 60 * 60 * 1000; // 60 min upper bound
  const t0 = Date.now();
  ...
  while (Date.now() - t0 < POLL_TIMEOUT_MS) {
    ...
    try {
      const r = await fetch(
        `/api/generate/tts-background?id=${encodeURIComponent(taskId)}`,
        { signal },
      );
      const data = (await readJsonResponse(r)) as {
        success?: boolean;
        status?: string;
        audioPath?: string;
        error?: string;
      ...
      if (!r.ok || !data?.success) {
        lastError = data?.error || `status ${r.status}`;
        continue;
      }
      status = data.status;
      audioPath = data.audioPath;
      lastError = data.error;
      if (status === 'completed' && audioPath) break;
      if (status === 'failed') {
        log.warn(`[TTS-bg] task ${taskId} failed for ${audioId}: ${lastError || 'unknown'}`);
        return;
      }
    } catch (err) {
  ...
  if (status !== 'completed' || !audioPath) {
    log.warn(
      `[TTS-bg] task ${taskId} for ${audioId} did not complete within ${POLL_TIMEOUT_MS / 60000}min (lastStatus=${status}, lastError=${lastError})`,
    );
    return;
  }
  try {
    const audioResp = await fetch(audioPath, { signal });
    if (!audioResp.ok) {
      log.warn(`[TTS-bg] failed to fetch ${audioPath}: ${audioResp.status}`);
      return;
    }
    const bytes = new Uint8Array(await audioResp.arrayBuffer());
    if (!bytes.length) {
      log.warn(`[TTS-bg] empty audio body from ${audioPath}`);
      return;
    }
    const fmt = audioPath.endsWith('.wav') ? 'wav' : 'bin';
    const blob = new Blob([bytes], { type: `audio/${fmt}` });
    const duration = measureAudioDuration(bytes, fmt) ?? undefined;
    ...
    log.info(`[TTS-bg] ${audioId} stored (${bytes.length} bytes from ${audioPath})`);
  } catch (err) {
    if (isAbortError(err)) return;
    log.warn(`[TTS-bg] failed to write ${audioId} to IndexedDB:`, err);
  }
}
      ...
      try {
        ...
        for (const outline of pending) {
          ...
          if (actionsResult.success && actionsResult.scene) {
            ...
            // TTS generation — async, non-blocking. CPU VoxCPM clone-mode
            // synthesis is 25-50 min/action, so we no longer let TTS failures
            // fail the whole scene. The text content is the source of truth;
            // TTS lands in IndexedDB on its own clock and the playback layer
            // picks it up when it arrives (or stays silent if it never does).
            if (
              settings.ttsEnabled &&
              ...
              const speechActions = (scene.actions || []).filter(
                (a: { type: string; text?: string; audioId?: string }) =>
                  a.type === 'speech' && !!a.text,
              );
              for (const a of speechActions) {
                if (!a.audioId) a.audioId = `tts_${a.id}`;
              }
              if (speechActions.length > 0) {
                log.info(
                  `[TTS-bg] queueing ${speechActions.length} TTS for scene ${scene.id} in background`,
                );
                void (async () => {
                  for (const action of speechActions) {
                    try {
                      ...
                      log.warn(
                        `[TTS-bg] failed for ${action.audioId}:`,
                        err,
                      );
                    }
                  }
  ...
  const retrySingleOutline = useCallback(
    async (outlineId: string) => {
      ...
      try {
        ...
        // Step 3: TTS (background, non-blocking — see comment in generateRemaining)
        const settings = useSettingsStore.getState();
        if (
          settings.ttsEnabled &&
          settings.ttsProviderId !== 'browser-native-tts' &&
            ...
            void (async () => {
              for (const action of speechActions) {
                try {
                  ...
                  log.warn(`[TTS-bg] retry failed for ${action.audioId}:`, err);
                }
              }
            })();
          }
